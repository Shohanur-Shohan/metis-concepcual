...
const DeleteServicePop = ({item}) => {

    const serviceID = item?.id;

    const handleDeleteBlog = async (ID) => {
        try {
            let res = await axios.delete(`/api/dashboard/service/delete?id=${ID}`);
            if(res?.status === 200) {
